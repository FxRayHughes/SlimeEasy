...
全部内容分入四个 Slimefun 分类: **实用机械** (自动化装置)、**实用工具** (手持辅助工具)、**存储系统** (海量存储与存储网络) 与 **简易村民** (与村民打交道而无需操心保护村民), 均需消耗经验研究解锁 (等级 8–30 不等)。
...
观察者外观, 脸朝向即攻击方向。随 Slimefun tick 自动对**脸朝向前方 3×3 区域** (随范围升级增大) 内的非玩家生物发动范围横扫。右键打开界面:
...
- **界面设置**: 左键 / 右键开关 (默认均关闭 → 不点击); 点击间隔可调 **0.05–40 tick** (普通点击 ±0.25, Shift 点击 ±0.05); 间隔 <1 tick 时单 tick 内连点多次
...
- 左键破坏方块 / 右键交互
- **抽取升级** (可选): 界面内可安装抽取升级, 补料源从"仅相邻漏斗"扩展为**相邻漏斗 / 箱子 / 本插件存储容器**, 每 tick 尽量补满物品槽, 并可配置黑 / 白名单与生效面

研究解锁: **15 级经验**。**合成** (增强工作台): 观察者居中, 红石供能, 金锭 + 金块 + 红石块环绕。

### 采石场

观察者外观。**脸朝向的圆石**若同时相邻**岩浆**与**水** (不必是源头), 便持续产出圆石 (不破坏该圆石)。本身**无容积**, 产物直接输出到周围的容器 / 抽屉 / 翻页箱。

- **效率升级 I~V** (放入右键界面的效率升级槽, 仅认物品身份, 与堆叠数量无关): 空槽 **1 个/秒**; I=1 · II=6 · III=12 · IV=32 · V=64 **个/0.5 秒**
...
- 每次激光造成**魔法 (药水) 伤害, 无视护甲**; 四档分别为 **5 / 10 / 20 / 25** 点
- 交战半径 16 格, 每只乐魂开火间隔约 1.5 秒
- 玩家与乐魂始终安全, 无需操心保护实体
...
- **抽取升级**: 每 tick 从相邻六向的漏斗 / 箱子 / 本插件存储容器主动提取物品入库; 可在 GUI 内配置黑 / 白名单与**生效面** (指定哪些方向生效)。亦可安装在**自动点击器**上作补料来源
- **物品输出升级**: 每 tick 把库存物品主动推送到相邻六向的容器 (箱子 / 漏斗 / 输出箱 / 本插件存储容器); 同样支持黑 / 白名单与生效面配置。以 **1 个物品输出箱 + 8 张纸**合成
- **翻页扩容**: 为翻页箱增加 1 页 (可叠装, 最多 5 页; 对抽屉无效)
- **智者 / 末影智者升级**: 经验容器吸入经验时有 20% / 50% 概率翻倍 (需与经验 + 磁铁升级同装)
- **远程升级**: 手持右键网络控制器选定目标, 装入抽屉 / 箱子后使其**远程接入**该控制器网络 (无视物理相邻范围)
...
│   └── quarry/               采石场 (圆石+岩浆+水产圆石 / 效率升级 / 输出到相邻容器)
