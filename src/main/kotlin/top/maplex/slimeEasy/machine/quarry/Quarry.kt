 ...
 * 采石场 (观察者方块)。
 *
 * 附着 (脸朝向) 的圆石同时相邻岩浆与水时, 持续产出圆石 (不破坏该圆石), 推送到本机
 * 周围的容器 / 抽屉 / 翻页箱。速率由升级槽内的效率组件决定 (见 [QuarryTier]):
...
class Quarry(
     ...
     *
     * 仅"无升级"档 (每 2 tick 产 1) 需要计数; 内存态即可 —— 重启后从 0 重数无副作用。
     */
    ...
    private fun onTick(quarry: Block) {
        ...
        QuarryMenuPreset.updateInfo(menu, attached = cobble != null, producing = producing, tier = tier)
        ...
        QuarryLogic.output(quarry, amount)
    }
    ...
    private companion object {
        /** 无升级档的产出节流 (Slimefun tick 数); 2 tick ≈ 1 秒。 */
        const val BASE_INTERVAL = 2

        /** 无升级档每次产出数量。 */
        const val BASE_PER_OPERATION = 1
    }
}
